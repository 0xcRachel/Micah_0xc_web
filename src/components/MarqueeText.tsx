...
export default function MarqueeText({
  ...
  }, containerRef, [direction, speed])

  const repeated = Array(6).fill(text)
  const dotColor = variant === 'bold' ? 'bg-led' : 'bg-led/30'
      ...
      <div data-marquee-track className="flex whitespace-nowrap">
        {repeated.map((t, i) => (
          <span key={i} className="flex items-center shrink-0">
            <span className={`mx-6 font-serif text-[inherit] ${variant === 'bold' ? 'opacity-5' : 'opacity-15'}`}>{t}</span>
